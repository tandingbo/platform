...
/**
 * ClassName: SqlScriptReader
 * Description: 定义一sql脚本读取器
 * Author: Tandingbo
...
public class SqlScriptReader {
    ...
    private String readLine() throws IOException {
        ...
        do {
            ...
            //如果为空行或单行注释
            if (StringUtils.isBlank(lineData) || StringUtils.startsWith(lineData, SingleLineCommentSeperator)) {
                continue;
            }
            //如果为多行注释
            if (StringUtils.startsWith(lineData, MultiLineCommentSeperator_Begin)) {
                //处理多行注释
